"use client";

import { useCallback } from "react";
import { useSortable } from "@dnd-kit/sortable";
import { CSS } from "@dnd-kit/utilities";
import MarkdownViewer from "./MarkdownViewer";
import InlineSectionEditor from "./InlineSectionEditor";
import type { MarkdownSection } from "@/lib/markdownSections";

interface SortableSectionCardProps {
  section: MarkdownSection;
  isActive: boolean;
  trashLabel?: string;
  isEditing: boolean;
  onStartEdit: (pageNumber: number) => void;
  onCancelEdit: () => void;
  onEditSection: (pageNumber: number, newRawContent: string) => void;
  onDeleteSection: (pageNumber: number) => void;
  setSectionRef: (page: number) => (el: HTMLDivElement | null) => void;
  highlightQuery?: string;
  highlightCaseSensitive?: boolean;
}

export default function SortableSectionCard({
  section,
  isActive,
  trashLabel,
  isEditing,
  onStartEdit,
  onCancelEdit,
  onEditSection,
  onDeleteSection,
  setSectionRef,
  highlightQuery,
  highlightCaseSensitive,
}: SortableSectionCardProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
    transform,
    transition,
    isDragging,
  } = useSortable({ id: section.pageNumber });

  const style = {
    transform: CSS.Transform.toString(transform),
    transition,
    opacity: isDragging ? 0.5 : 1,
  };

  const isHeader = section.pageNumber === 0;

  // Combine dnd-kit ref and scroll-sync ref
  const combinedRef = useCallback(
    (el: HTMLDivElement | null) => {
      setNodeRef(el);
      setSectionRef(section.pageNumber)(el);
    },
    [setNodeRef, setSectionRef, section.pageNumber]
  );

  return (
    <div
      ref={combinedRef}
      data-section={section.pageNumber}
      style={style}
      className={`rounded-lg border bg-white p-4 transition-all ${
        isActive
          ? "ring-2 ring-indigo-500 border-indigo-300"
          : "border-slate-200 hover:border-slate-300"
      }`}
    >
      {/* Card header */}
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          {/* Drag handle */}
          <button
            {...attributes}
            {...listeners}
            className="cursor-grab active:cursor-grabbing p-1 text-slate-300 hover:text-slate-500 transition-colors touch-none"
            title="Drag to reorder"
          >
            <svg className="w-4 h-4" viewBox="0 0 16 16" fill="currentColor">
              <circle cx="5" cy="3" r="1.5" />
              <circle cx="11" cy="3" r="1.5" />
              <circle cx="5" cy="8" r="1.5" />
              <circle cx="11" cy="8" r="1.5" />
              <circle cx="5" cy="13" r="1.5" />
              <circle cx="11" cy="13" r="1.5" />
            </svg>
          </button>
          <span className="text-xs font-semibold text-slate-500 uppercase tracking-wide">
            {isHeader ? "Header" : `Page ${section.pageNumber}`}
          </span>
          {trashLabel && (
            <span className="px-1.5 py-0.5 text-[10px] font-bold rounded bg-amber-100 text-amber-700">
              {trashLabel}
            </span>
          )}
        </div>
        <div className="flex items-center gap-1">
          {/* Edit button */}
          {!isHeader && !isEditing && (
            <button
              onClick={() => onStartEdit(section.pageNumber)}
              className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded transition-colors"
              title={`Edit page ${section.pageNumber}`}
            >
              <svg
                className="w-4 h-4"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={2}
                stroke="currentColor"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10"
                />
              </svg>
            </button>
          )}
          {/* Delete button */}
          {!isHeader && (
            <button
              onClick={() => {
                if (
                  confirm(
                    `Remove Page ${section.pageNumber} section from the markdown?`
                  )
                ) {
                  onDeleteSection(section.pageNumber);
                }
              }}
              className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded transition-colors"
              title={`Delete page ${section.pageNumber}`}
            >
              <svg
                className="w-4 h-4"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={2}
                stroke="currentColor"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"
                />
              </svg>
            </button>
          )}
        </div>
      </div>

      {/* Card body */}
      <div className="max-h-[300px] overflow-y-auto">
        {isEditing ? (
          <InlineSectionEditor
            rawContent={section.rawContent}
            onSave={(newContent) =>
              onEditSection(section.pageNumber, newContent)
            }
            onCancel={onCancelEdit}
          />
        ) : section.rawContent ? (
          <MarkdownViewer
            content={section.rawContent}
            highlightQuery={highlightQuery}
            highlightCaseSensitive={highlightCaseSensitive}
          />
        ) : (
          <p className="text-xs text-slate-400 italic">Empty section</p>
        )}
      </div>
    </div>
  );
}
